...
all <- bind_rows(states9, states10, states11) %>%
...
tbl <- pos %>%
  ...
  # which is COW, not G&W coding
  filter(v9!="first year of independence") %>%
  mutate(v9 = recode(v9), v11 = recode(v11)) %>%
  with(., table(v9 = v9, v11 = v11))
as_tibble(tbl) %>%
  pivot_wider(names_from = "v11", values_from = "n") %>%
  kbl(booktabs = TRUE, format = "latex", label = "v9-v11-comp",
      caption = "Comparison between v9 and v11 data positive cases") %>%
  kable_styling() %>%
  add_header_above(c(" " = 1, "v11" = 3)) %>%
  writeLines(here::here("2021-update/output/table-v9-v11.tex"))

#
#   Agreement rates table for different comparisons
#

pos_long <- pos %>%
  pivot_longer(c(v9, v10, v11), names_to = "version", values_to = "change")
to <- pos_long %>%
...
comp <- full_join(pos_long, to) %>%
  ...
  filter(comparison %in% c("v9-v10", "v10-v11", "v9-v11")) %>%
  select(-version, -version2)
# Sanity check: since I'm making 3 comparison, i should have 3 times the rows
# as pos
stopifnot(nrow(comp)==3*nrow(pos))
