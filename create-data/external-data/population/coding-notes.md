...
-   [Expanded population data from KSG](#expanded-population-data-from-ksg)
-   [WDI pop data](#wdi-pop-data)
-   [UN pop data](#un-pop-data)
    -   [Reconstruct some countries that later split](#reconstruct-some-countries-that-later-split)
-   [Combined and overlap](#combined-and-overlap)
    -   [Within country covariances between UN and KSG](#within-country-covariances-between-un-and-ksg)
    -   [Can we combine KSG for pre-50 with UN?](#can-we-combine-ksg-for-pre-50-with-un)
...
-   [Get ready to write final data](#get-ready-to-write-final-data)
    -   [Check values for splitting/joining countries](#check-values-for-splittingjoining-countries)
...
``` r
ksg$date <- as.Date(paste0(ksg$year, "-12-31"))
plot_missing(ksg, "pop", "idnum", "date", "year", "GW")
```
...
wdi$date <- as.Date(paste0(wdi$year, "-12-31"))
plot_missing(wdi, "pop", "gwcode", "date", "year", "GW")
```
...
The data are from <https://population.un.org/wpp/DataQuery/>. Since only a max of 35 years can be downloaded at a time, there are two files needed as input.
...
``` r
gw <- state_panel("1950-01-01", "2019-12-31", "year", partial = "any") %>%
  mutate(year = lubridate::year(date))
gw_not_in_un <- gw %>%
  anti_join(un, by = c("gwcode", "year")) %>%
  group_by(gwcode) %>%
  mutate(seq = id_date_sequence(date, "year")) %>%
  group_by(gwcode, seq) %>%
...
un_not_in_gw <- un %>%
  ...
  mutate(date = as.Date(paste0(year, "-12-31"), format = "%Y-%m-%d")) %>%
  group_by(gwcode) %>%
  mutate(seq = id_date_sequence(date, "year")) %>%
  group_by(gwcode, seq) %>%
  summarize(years = paste0(range(year), collapse = " - ")) %>%
...
un %>%
  ...
  as.data.frame() %>%
  plot_missing(., "pop", "gwcode", "date", "year", "GW")
```
...
```
    ...
    ## Warning: Removed 25175 rows containing missing values (geom_path).
...
![](coding-notes_files/figure-markdown_github/unnamed-chunk-16-1.png)
...
``` r
master <- state_panel("1816-01-01", paste0(max(joint$year), "-12-31"), "year",
                      partial = "any") %>%
  mutate(year = as.integer(substr(date, 1, 4)), date = NULL, gw = TRUE)
pop <- joint_wide %>%
  mutate(
    pop = case_when(
      year > 1949 ~ pop_un,
...
```
    ...
    ##   gwcode  year source_ksg overlap pop_ksg pop_un pop_wdi
    ##    <dbl> <dbl>      <dbl> <lgl>     <dbl>  <dbl>   <dbl>
    ## 1    816  1971          0 FALSE     21595 44486.     NA 
    ## 2    816  1972          0 FALSE     22038 45549.     NA 
    ## 3    816  1973          0 FALSE     22481 46605.     NA 
    ## 4    816  1974          0 FALSE     23244 47662.     NA 
    ## 5    816  1975          0 TRUE      24032 48729.  48729.
    ## 6    816  1976          0 TRUE      49160 49808.  49808.
    ## 7    816  1977          0 TRUE      50413 50900.  50900.
    ## 8    816  1978          0 TRUE      51423 52015.  52015.
    ## 9    816  1979          0 TRUE      52462 53170.  53170.
...
```
    ...
    ##   gwcode  year source_ksg overlap pop_ksg pop_un pop_wdi
    ##    <dbl> <dbl>      <dbl> <lgl>     <dbl>  <dbl>   <dbl>
    ## 1    678  1986          0 FALSE      7911 10233.     NA 
    ## 2    678  1987          0 FALSE      8213 10629.     NA 
    ## 3    678  1988          0 FALSE      8529 11052.     NA 
    ## 4    678  1989          0 FALSE      8857 11523.     NA 
    ## 5    678  1990          0 TRUE       9196 12057.  12057.
    ## 6    678  1991          0 TRUE      11613 12662.  12662.
    ## 7    678  1992          0 TRUE      11952 13326.  13326.
    ## 8    678  1993          0 TRUE      12302 14017.  14017.
    ## 9    678  1994          0 TRUE      14859 14693.  14693.
...
```
    ...
    ##   gwcode  year source_ksg overlap pop_ksg pop_un pop_wdi
    ##    <dbl> <dbl>      <dbl> <lgl>     <dbl>  <dbl>   <dbl>
    ## 1    347  2009         NA FALSE        NA     NA   1761.
    ## 2    347  2010         NA FALSE        NA     NA   1776.
    ## 3    347  2011         NA FALSE        NA     NA   1791 
    ## 4    347  2012         NA FALSE        NA     NA   1805.
    ## 5    347  2013         NA FALSE        NA     NA   1824.
    ## 6    347  2014         NA FALSE        NA     NA   1822.

``` r
# Get Kosovo series and kalman smooth
idx <- pop$gwcode==347
kos <- pop$pop_wdi[idx]
kos <- imputeTS::na.kalman(kos, "auto.arima")
pop$pop[idx] <- kos
pop$source[idx] <- "wdi"
```
...
```
    ...
    ## 1    315  1918         NA NA           NA     NA      NA    NA <NA>   TRUE 
    ## 2    315  1919          0 FALSE     13398     NA      NA 13398 ksg    TRUE 
    ## 3    315  1920          0 FALSE     13530     NA      NA 13530 ksg    TRUE
...
![](coding-notes_files/figure-markdown_github/plot-czechoslovakia-1.png)
...
![](coding-notes_files/figure-markdown_github/unnamed-chunk-24-1.png)
...
![](coding-notes_files/figure-markdown_github/unnamed-chunk-25-1.png)

### Missingness

``` r
pop$date <- as.Date(paste0(pop$year, "-12-31"))
pop %>%
  plot_missing(., "pop", "gwcode", "date", "year", "GW")
```

![](coding-notes_files/figure-markdown_github/unnamed-chunk-26-1.png)
